...
mounted=()
cleanup() {
  for dest in "${mounted[@]}"; do sudo umount "$dest" || true; done
...
sudo chroot "$root" /usr/bin/qemu-aarch64-static /bin/sh -c \
  "DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends cuda-libraries-dev libnvinfer-dev libopencv-dev libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev libavformat-dev libavcodec-dev libavutil-dev libswscale-dev"
cleanup
trap - EXIT
mounted=()
...
sudo apt-get update
sudo DEBIAN_FRONTEND=noninteractive apt-get install -y cuda-nvcc cuda-cross-aarch64
nvcc='/usr/local/cuda/bin/nvcc'
[[ -x $nvcc ]] || die "Host nvcc missing at $nvcc"
...
[[ -f $root/usr/include/NvInfer.h ]] || die 'NvInfer.h missing after ARM64 apt install'
printf 'int main(){return 0;}\n' | "${cross}g++" --sysroot="$root" -x c++ - -o "$sdk/hello-aarch64"
...
echo "cmake -S PROJECT -B BUILD -G Ninja -DCMAKE_TOOLCHAIN_FILE='$sdk/toolchain.cmake'"
